...
Finally, we can get predictions at new data points (or the training data) using `predict`, which actually outputs samples from the . Optionally, prediction intervals can be estimated using the (plug-in) predictive distribution at the MLEs (see ```?predict.lmstar``` for details). Note that "plug-in" predictive distribution is a crude approximation, and better approaches for uncertainty quantification are available using the Bayesian models.
```{r predict-lm}
...
In addition to the linear model, `rSTAR` also has implementations for STAR models paired with more flexible regression methods, in particular random forests and generalized boosted machines.  

### A Generic EM Algorithm
The EM algorithm updates the parameters until convergence to the maximum likelihood estimators (MLEs), and is implemented in `genEM_star()`:
...
fit_mcmc = genMCMC_star(y = y,
                          sample_params = function(y, params)
                            rSTAR:::sample_lm_ridge(y, X, params),
                         init_params = function(y) 
                           rSTAR:::init_lm_ridge(y,X),
