...
In addition to the linear model, `rSTAR` also has implementations for STAR models paired with more flexible regression methods, in particular random forests (`randomForest_star()`) and generalized boosted machines (`gbm_star()`). In these functions, the user directly inputs the set of predictors $X$ alongside any test points in $X_test$, as can be seen in the example below
```{r freq-ml}
# Select a transformation:
transformation = 'np' # Estimated transformation using empirical CDF
