...
a STAR linear regression model with a g-prior and Bayesian nonparametric
(BNP) transformation.
...
and then uses a Gibbs sampler for the latent data `z` and the regression
coefficients `beta`. Compared to the direct Monte Carlo version in
[blm_star_exact](https://bking124.github.io/countSTAR/reference/blm_star_exact.md),
this MCMC algorithm uses a blocking structure that balances MCMC
efficiency (the marginal sampler for `g`) with computational scalability
(the Gibbs sampler for `z` and `beta`).
