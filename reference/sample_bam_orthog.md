...
orthogonalized splines. The sampler draws the linear terms jointly and
then samples each vector of nonlinear coefficients using Bayesian
...
- diagBtB_all:
  ...
  optional `pNL`-dimensional list of `diag(crossprod(B_all[[j]]))`; if
  NULL, compute internally
